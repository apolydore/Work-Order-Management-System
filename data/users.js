...
const checkAlpha = (val, varName) => {
  ...
  if (!/^[A-Za-z]+$/.test(trimmed))
    throw `${varName} must contain only alphabetic characters`;
  return trimmed;
};
...
const checkName = (name, fieldName) => {
  ...
  if (!/^[A-Za-z \-]+$/.test(trimmed))
    throw `${fieldName} may only contain letters, spaces, and hyphens`;
  if (!/[A-Za-z]/.test(trimmed))
    throw `${fieldName} must contain at least one letter`;
  if (trimmed.length < 2 || trimmed.length > 50)
    throw `${fieldName} must be between 2 and 50 characters long`;
  return trimmed;
};
...
const checkRole = (role) => {
  ...
  if (!allowedRoles.includes(trimmed))
    throw 'role must be either "admin" or "contractor"';
  return trimmed;
};
...
const checkCity = (city) => {
  ...
  if (!/^[A-Za-z .]+$/.test(trimmed))
    throw "city may only contain letters, spaces, and period";
...
const checkState = (state) => {
  ...
  if (!/^[A-Za-z]{2}$/.test(trimmed))
    throw 'state must be two alphabetic characters (e.g. "NJ")';
...
const checkPhone = (phone) => {
  ...
  if (!/^\d{3}-\d{3}-\d{4}$/.test(trimmed))
    throw "phone number must be in the format of 000-000-0000";
  return trimmed;
};
...
exportedMethods.createUser = async (
  role,
  firstName,
  lastName,
  city,
...
exportedMethods.getAllContractors = async () => {
  const usersCol = await users();
  const contractors = await usersCol.find({ role: "contractor" }).toArray();
  return contractors;
};

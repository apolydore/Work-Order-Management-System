...
const ensureActiveCompany = async (companyName) => {
  ...
  const doc = await col.findOne({ companyName: v.checkString(companyName, 'companyName') });
...
exportedMethods.createJobRequest = async (
  ...
  if (!insertInfo.acknowledged || !insertInfo.insertedId) throw 'could not create job request';
...
exportedMethods.getAllJobRequests = async (filters = {}) => {
  ...
  if (filters.companyName) query.companyName = v.checkString(filters.companyName, 'companyName');
  if (filters.priority) query.priority = v.priority(filters.priority);
  if (filters.category) query.category = v.checkString(filters.category, 'category');
...
exportedMethods.updateJobRequest = async (id, updates) => {
  ...
  if (updates.companyName !== undefined) {
    ...
    toSet.comanyName = companyDoc.companyName;
    toSet.comapnyId = companyDoc._id;
  }
  if (updates.category !== undefined) toSet.category = v.checkString(updates.category, 'category');
  if (updates.priority !== undefined) toSet.priority = v.priority(updates.priority);
  if (updates.description !== undefined) toSet.description = v.checkString(updates.description, 'description');
  if (updates.address !== undefined) toSet.address = v.checkString(updates.address, 'address');
  if (updates.city !== undefined) toSet.city = v.checkString(updates.city, 'city');
  if (updates.state !== undefined) toSet.state = v.checkState(updates.state, 'state');
  if (updates.zipCode !== undefined) toSet.zipCode = v.checkZip(updates.zipCode, 'zipCode');
      if (updates.status !== undefined) toSet.status = v.statusJR(updates.status);
  if (updates.requestedDate !== undefined) toSet.requestedDate = v.checkDate(updates.requestedDate);
